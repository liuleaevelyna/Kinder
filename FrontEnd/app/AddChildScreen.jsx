...
const SKILL_CATEGORIES = [
  { id: 1, label: "Școală", icon: "book-outline", color: COLORS.scoala },
  {
    id: 2,
    label: "Disciplină",
    icon: "alarm-outline",
    color: COLORS.disciplina,
  },
  { id: 3, label: "Respect", icon: "heart-outline", color: COLORS.respect },
  { id: 4, label: "Sport", icon: "bicycle-outline", color: COLORS.sport },
  { id: 5, label: "Ordine", icon: "home-outline", color: COLORS.ordine },
  { id: 6, label: "Altele", icon: "star-outline", color: COLORS.primary },
];
...
export default function AddChildScreen() {
  ...
  const [prag, setPrag] = useState("85");
  const [loading, setLoading] = useState(false);
  const [eroriPas1, setEroriPas1] = useState({});
            ...
            {[1, 2, 3].map((i) => (
              <View key={i} style={styles.stepItem}>
                <View style={[styles.stepCircle, pas >= i && styles.stepActiv]}>
                  <Text
                    style={[styles.stepNum, pas >= i && { color: "white" }]}
                  >
                    {i}
                  </Text>
                </View>
                {i < 3 && (
                  <View
                    style={[styles.stepLine, pas > i && styles.stepLineActiv]}
                  />
                )}
              </View>
            ))}
        ...
        >
          ...
          {pas === 1 && (
            <View style={styles.card}>
              <View style={styles.row}>
                <View style={{ flex: 1 }}>
                  ...
                  <View
                    style={[
                      styles.inputBox,
                      eroriPas1.prenumeC && { borderColor: COLORS.error },
                    ]}
                  >
                    <TextInput
                      style={styles.input}
                      ...
                      onChangeText={(t) => {
                        setPrenumeC(t);
                        setEroriPas1({ ...eroriPas1, prenumeC: null });
                      }}
                    />
                  </View>
                  {eroriPas1.prenumeC && (
                    <Text style={styles.eroare}>{eroriPas1.prenumeC}</Text>
                  )}
                </View>
                <View style={{ width: 12 }} />
                <View style={{ flex: 1 }}>
                  ...
                  <View
                    style={[
                      styles.inputBox,
                      eroriPas1.numeC && { borderColor: COLORS.error },
                    ]}
                  >
                    <TextInput
                      style={styles.input}
                      ...
                      onChangeText={(t) => {
                        setNumeC(t);
                        setEroriPas1({ ...eroriPas1, numeC: null });
                      }}
                    />
                  </View>
                  {eroriPas1.numeC && (
                    <Text style={styles.eroare}>{eroriPas1.numeC}</Text>
                  )}
                </View>
              </View>
              ...
              <View
                style={[
                  styles.inputBox,
                  eroriPas1.varsta && { borderColor: COLORS.error },
                ]}
              >
                <TextInput
                  style={styles.input}
                  ...
                  onChangeText={(t) => {
                    setVarsta(t);
                    setEroriPas1({ ...eroriPas1, varsta: null });
                  }}
              ...
              {eroriPas1.varsta && (
                <Text style={styles.eroare}>{eroriPas1.varsta}</Text>
              )}
              ...
              <View style={styles.sexRow}>
                {SEX_OPTIONS.map((opt) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.sexBtn,
                      sex === opt.value && styles.sexBtnActiv,
                      eroriPas1.sex && { borderColor: COLORS.error },
                    ]}
                    onPress={() => {
                      setSex(opt.value);
                      setEroriPas1({ ...eroriPas1, sex: null });
                    }}
                  >
                    <Text
                      style={[
              ...
              {eroriPas1.sex && (
                <Text style={styles.eroare}>{eroriPas1.sex}</Text>
              )}
          ...
          {pas === 2 && (
            <View style={styles.card}>
              ...
              <Text style={styles.cardSubtitle}>
                Selectează domeniile — aplicația va genera taskuri automat
              </Text>
                  ...
                  return (
                    ...
                    >
                      <Ionicons
                        name={cat.icon}
                        size={28}
                        color={selectat ? cat.color : COLORS.textMedium}
                        style={{ marginBottom: 4 }}
                      />
                      <Text
                        style={[
                          styles.skillLabel,
                          selectat && { color: cat.color, fontWeight: "800" },
                        ]}
                      >
                        {cat.label}
                      </Text>
          ...
          {pas === 3 && (
            <View style={styles.card}>
              ...
              <Text style={styles.cardSubtitle}>
                Copilul o primește dacă atinge pragul setat
              </Text>
              ...
              <View style={styles.inputBox}>
                ...
                <TextInput
                  style={styles.input}
                  placeholder="ex: Bicicletă, Tabletă, Joc"
                  placeholderTextColor={COLORS.textLight}
                  value={premiu}
                  onChangeText={setPremiu}
                />
              </View>
              ...
              <View style={styles.duratRow}>
                {["7", "14", "30", "60"].map((d) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.duratBtn,
                      durataCiclu === d && styles.duratBtnActiv,
                    ]}
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.duratSub,
                        durataCiclu === d && { color: "rgba(255,255,255,0.8)" },
                      ]}
                    >
                      zile
                    </Text>
              ...
              <Text style={styles.label}>Pragul de succes *</Text>
              <View style={styles.duratRow}>
                {["70", "75", "80", "85", "90", "100"].map((p) => (
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.duratSub,
                        prag === p && { color: "rgba(255,255,255,0.8)" },
                      ]}
                    >
                      %
                    </Text>
          ...
          <View style={styles.btnRow}>
            ...
            {pas < 3 ? (
              <TouchableOpacity
                style={[styles.btnNext, { flex: 1 }]}
                onPress={() => {
                  if (pas === 1) {
                    let e = {};
                    if (!prenumeC.trim())
                      e.prenumeC = "Prenumele este obligatoriu";
                    if (!numeC.trim()) e.numeC = "Numele este obligatoriu";
                    if (!varsta.trim()) e.varsta = "Vârsta este obligatorie";
                    else if (
                      isNaN(varsta) ||
                      parseInt(varsta) < 1 ||
                      parseInt(varsta) > 18
                    )
                      e.varsta = "Vârsta trebuie să fie între 1 și 18 ani";
                    if (!sex) e.sex = "Selectează genul copilului";
                    if (Object.keys(e).length > 0) {
                      setEroriPas1(e);
                      Alert.alert(
                        "Câmpuri incomplete",
                        "Te rugăm să completezi toate câmpurile obligatorii marcate cu *",
                      );
                      return;
                    }
                    setEroriPas1({});
